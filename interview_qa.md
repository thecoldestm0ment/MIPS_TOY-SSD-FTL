...
### Q18. 직접 코드를 짰는지 확인하려고 `run_simple_gc`를 물어보면 어떻게 설명하면 되나요?

A. `run_simple_gc`는 PBA 상태 배열을 처음부터 끝까지 돌면서 INVALID인 항목만 FREE로 바꿉니다. 바꾼 개수를 `freed`에 세고, free page count와 invalid page count를 조정한 뒤 GC 횟수와 trace를 기록합니다.
